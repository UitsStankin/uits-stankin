 ...
 *
 * «Один раз» держится на том, что признак сессии к этому моменту ещё жив,
 * и снимает его отсюда только первый пришедший. Поэтому `refreshAccessToken`
 * сессию не трогает: пока трогал, охрана ниже съедала не лишние вызовы,
 * а единственный, и на форму входа не уходил никто.
 */
...
async function exchangeRefreshCookie(): Promise<RefreshResult> {
  try {
    ...
    //
    // Сессия здесь НЕ гасится, хотя соблазн велик: обмен только сообщает,
    // чем кончилось, а хоронит сессию вызывающий. Гасить здесь — значит
    // отнять у `expireSession` признак, по которому тот отличает первого
    // из налетевших запросов от остальных: `hasSession()` к его вызову уже
    // был бы `false`, и увести на форму входа не успел бы никто.
    // Разные вызывающие и решают по-разному: интерцептор уводит на логин,
    // восстановление сессии при запуске вкладки — просто забывает признак,
    // потому что «cookie не сработала» на старте означает «пришли гостем».
