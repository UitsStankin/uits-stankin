 ...
 * - **порядок `?sort=createdAt,asc|desc`** — да, потому что раздел админки
 *   умеет менять его кликом по заголовку. Сортировки по другим полям нет:
 *   их не просит ни одна страница, а мок, отвечающий на то, чего не бывает,
 *   сторожил бы несуществующий код;
...
export function newsHandlers(items: readonly News[] = newsFixture) {
  ...
  return [
    http.get(PUBLIC_NEWS, ({ request }) => {
      ...
      return HttpResponse.json(pageFromUrl(filterByType(visible, url), url));
    }),
 ...
 * колонка админского списка — дата (`useNewsAdmin`).
 */
