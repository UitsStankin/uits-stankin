 ...
 * и со словарём `errors` из одного ключа.
 *
 * Форма ответа — та, что приехала с T-80 (`FieldValidationException`,
 * 2026-09-10, заявка **B-6**): до неё сервис отвечал одним `detail`,
 * и форме было нечем понять, к какому полю относится сообщение.
 * Словарь тот же текст повторяет — так отвечает и сервер: `detail`
 * читает человек, `errors` разбирает форма.
 *
 * Список, а не строка: `parseErrors` берёт только массивы строк,
 * и `{"name": "текст"}` он молча выбросит — отказ снова уехал бы
 * в баннер, причём беззвучно.
 */
function duplicateName(instance: string, name: string) {
  const message = `Дисциплина с названием «${name}» уже существует`;
