 ...
 *
 * `input[type=file]` исключён намеренно, и это не про табуляцию, а про
 * первый фокус при открытии. Выбор файла в портале — всегда скрытый
 * `sr-only` input под видимой подписью (`shared/ui/ImagePicker`), потому
 * что родного «Выберите файл» не стилизовать. В разметке он идёт первым,
 * и окно, открытое формой с фотографией сверху, отдавало фокус ему:
 * диктор говорил «Выбрать фото», а зрячий с клавиатуры не видел кольца
 * нигде — элемент размером в пиксель. Найдено на живом стенде в форме
 * УВП (D-F15, 2026-09-10).
 *
 * Из обхода табом он при этом не выпадает: `<label>` вокруг него отдаёт
 * ему и клик, и фокус, а замыкание табуляции считается по этому же
 * списку — то есть выбор файла остаётся достижимым, просто не первым.
 */
const FOCUSABLE =
