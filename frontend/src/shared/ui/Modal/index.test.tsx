...
describe('Modal', () => {
  ...
  /**
   * Скрытый выбор файла фокус не забирает.
   *
   * Выбор картинки в портале — всегда `sr-only` input под видимой
   * подписью (`shared/ui/ImagePicker`), и в разметке форм с фотографией
   * он идёт первым. Окно отдавало фокус ему: диктор говорил «Выбрать
   * фото», а зрячий с клавиатуры не видел кольца нигде — элемент
   * размером в пиксель. Найдено на живом стенде в форме УВП (D-F15).
   */
  it('пропускает скрытый выбор файла и берёт следующее поле', () => {
    render(
      <Modal title="Новая карточка" onClose={() => {}}>
        <form>
          <label>
            Выбрать фото
    ...
    expect(screen.getByLabelText('Фамилия')).toHaveFocus();
    expect(screen.getByLabelText('Выбрать фото')).not.toHaveFocus();
  });
