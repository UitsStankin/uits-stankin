/**
 * Профиль пользователя и тела ручек аутентификации.
 *
 * Формы здесь описывают **контракт Spring** (`UserResponseDto`,
 * `AuthController.LoginRequest/LoginResponse`, `ChangePasswordRequest`),
 * а не то, что отдавал dj-rest-auth. Прежняя версия файла была скопирована
 * из Angular-оригинала вместе с джанговскими `pk`, `isSuperuser`,
 * `isModerator`, `isStaff` и `isAnonymous` — контракт разошёлся бы
 * с реальностью на первом же запросе к `/api/users/profile`.
 */

/**
 * Текущий пользователь — ответ `GET /api/users/profile`.
 *
 * Роли приходят тремя булевыми полями **без префикса `is`** (docs/API.md,
 * «Нюансы, которые из Swagger не видны»): в базе колонки остались
 * джанговскими (`is_superuser`), но наружу Spring отдаёт `superuser`.
 *
 * Чего в контракте нет и не надо ждать:
 *
 * - `isStaff` — «доступ в админку Django». Колонка `is_staff` в таблице
 *   ещё есть, но в DTO её нет: собственная админка (F-4x) правами
 *   не пользуется, у неё свои — админ или модератор.
 * - `isAnonymous` — джанговский `AnonymousUser`. В новом контракте
 *   неавторизованный пользователь не получает профиль вовсе: `/profile`
 *   отвечает `401`. Поэтому «не вошёл» — это отсутствие профиля
 *   (`Profile | null`), а не профиль с поднятым флагом. Фальшивый объект
 *   опасен ровно тем, что выглядит как настоящий: любое место, забывшее
 *   проверить флаг, покажет анонима как вошедшего.
 * - `telegramCode` и `telegramUser` — модуля Telegram на бэкенде нет
 *   (матрица паритета, п. 20). Формы лежат в `planned.types.ts`.
 */
export type Profile = {
  id: number;
  /** Логин. Единственное поле имени, которое гарантированно непустое. */
  username: string;
  /** Может не быть заполнено — тогда `null`. */
  firstName: string | null;
  lastName: string | null;
  email: string | null;
  /**
   * **Ключ** файла аватара в хранилище — тот, что вернул `POST /api/files`
   * с `category: avatars`. Уходит обратно в `PUT /api/users/profile`,
   * для показа не годится.
   */
  avatar: string | null;
  /**
   * Готовый адрес аватара для `<img src>`, относительный.
   *
   * Пара «ключ и адрес» здесь ровно та же, что у обложки новости
   * (`previewImage` / `previewImageUrl`), и путать их так же нельзя:
   * ключ в `src` даёт битую картинку. Без аватара оба поля `null`.
   */
  avatarUrl: string | null;
  /**
   * Владелец карточки преподавателя. Прав не добавляет: на закрытых ручках
   * преподаватель получает `403` наравне с обычным пользователем.
   */
  teacher: boolean;
  /** Модератор: управление новостями и загрузка файлов. */
  moderator: boolean;
  /** Суперпользователь. Сегодня по правам неотличим от модератора. */
  superuser: boolean;
};

/**
 * Тело `POST /api/users/auth/login`.
 *
 * Вход по **логину**, а не по почте: у прежней формы поле называлось `email`,
 * и с ним запрос не прошёл бы валидацию.
 */
export type LoginRequest = {
  username: string;
  password: string;
};

/**
 * Ответ `POST /api/users/auth/login` и `POST /api/users/auth/refresh` —
 * тело у них одно и то же, поэтому и тип один.
 *
 * Access-токен живёт **15 минут** и в теле ответа приезжает целиком.
 * Второго токена здесь нет и быть не должно: refresh уезжает
 * заголовком `Set-Cookie`, помечен `HttpOnly` и коду не виден вовсе —
 * ни прочитать, ни отправить его руками нельзя, этим занимается браузер.
 */
export type AccessTokenResponse = {
  accessToken: string;
};

/**
 * Тело `POST /api/users/change-password`. Успех — `200` с пустым телом.
 *
 * Неверный старый пароль приходит как `400`, а не `401`: это ошибка формы,
 * из личного кабинета на логин не выкидывает.
 */
export type ChangePasswordRequest = {
  oldPassword: string;
  /** Минимум 8 символов — проверяется и на бэкенде. */
  newPassword: string;
};

/*
 * Регистрации в контракте нет: `AuthController` знает только `/login`,
 * учётные записи заводит администратор. Поэтому прежние `RegisterForm`
 * и `ListUsersParams` (с джанговскими `is_moderator`, `is_teacher`
 * в snake_case) удалены, а не переписаны — им не соответствует ни одна ручка.
 */

/**
 * Тело `PUT /api/users/profile`. Ответ — профиль целиком, тот же `Profile`,
 * что отдаёт `GET`.
 *
 * Полей ровно три, и все три обязаны быть в теле: запрос **заменяет карточку
 * целиком**, а поле, которого в нём нет или которое пришло `null`, очищается.
 * `PUT` с одним лишь `avatar` стёр бы имя и фамилию (docs/API.md, «Профиль
 * пользователя»). Поэтому в типе они не необязательные — `?` здесь означал бы
 * «можно не слать», а слать нужно всегда.
 *
 * Логина, почты и флагов ролей здесь нет не по забывчивости: бэкенд их в теле
 * молча игнорирует и отвечает успехом. Объявленное поле означало бы форму,
 * которая делает вид, что что-то меняет.
 */
export type ProfileUpdateRequest = {
  /** До 150 символов; `null` — очистить. Пустая строка равносильна `null`. */
  firstName: string | null;
  lastName: string | null;
  /**
   * Ключ файла из раздела `avatars` — тот, что вернул `POST /api/files`.
   * Ключ чужого раздела отвергается с `400`, даже если файл существует.
   * `null` очищает аватар и удаляет файл с диска.
   */
  avatar: string | null;
};

/**
 * Чего требует раздел админки от вошедшего.
 *
 * Два значения, а не список ролей: права в контракте даёт пара
 * админ/модератор, и разделы делятся ровно надвое — контент (новости,
 * карточки, дисциплины, разделы сайта) доступен обоим, учётные записи —
 * только админу (docs/API.md, «Роли»).
 *
 * `moderator` здесь означает «хватит модератора», а не «только модератор»:
 * админ может всё, что может модератор, и отдельного значения «оба»
 * заводить незачем.
 */
export type AdminAccess = 'admin' | 'moderator';

/**
 * Запись справочника учётных записей — элемент `GET /api/users/directory`.
 *
 * Полей ровно три, и это не урезанный `Profile`, а свой DTO: ручка открыта
 * не только админу, но и модератору с преподавателем, поэтому логина
 * и почты в ней нет намеренно. «Перебор страниц с `username` в ответе
 * выгружал бы логины портала в обход админской проверки» (docs/API.md,
 * «Справочник учёток преподавателей») — та же утечка, которую закрыли
 * в карточках календаря.
 *
 * В выдачу попадают действующие учётки с ролью `teacher` — те, которым
 * можно что-то назначить. Форма карточки ППС берёт отсюда `userId`
 * (F-44): учётка без роли всё равно получила бы `400` при сохранении.
 *
 * ФИО может не быть заполнено — тогда обе строки `null`, и подпись
 * в списке собирает вызывающий.
 */
export type UserDirectoryEntry = {
  /** `id` учётной записи. Идентификатору карточки ППС он не равен. */
  id: number;
  lastName: string | null;
  firstName: string | null;
};
