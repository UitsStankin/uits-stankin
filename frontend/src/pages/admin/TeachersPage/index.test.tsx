...
describe('AdminTeachersPage, удаление', () => {
  ...
  /**
   * Учётная запись переживает удаление карточки, и предупреждение об этом
   * — часть подтверждения: модератор, убирающий карточку уволившегося,
   * иначе решил бы, что заодно закрыл ему вход.
   */
  it('предупреждает, что учётная запись останется', async () => {
    renderTeachers();
    ...
    expect(
      screen.getByText(/Учётная запись, если она была привязана, останется/),
    ).toBeInTheDocument();
  });
