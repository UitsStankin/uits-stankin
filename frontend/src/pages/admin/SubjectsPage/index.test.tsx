...
describe('SubjectsPage, создание', () => {
  ...
  /**
   * Занятое название приходит как `409` с общим текстом «Конфликт
   * данных.» — так отвечает уникальный индекс базы. Показывать его
   * дословно нельзя: из него не следует ни что случилось, ни что делать.
   */
  it('переводит отказ по занятому названию на поле формы', async () => {
    renderSubjects();
    ...
    expect(await screen.findByText('Дисциплина с таким названием уже есть')).toBeInTheDocument();
    expect(screen.getByRole('dialog')).toBeInTheDocument();
  });
