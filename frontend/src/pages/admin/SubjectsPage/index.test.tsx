...
describe('SubjectsPage, создание', () => {
  ...
  /**
   * Занятое название приходит как `400` с готовым текстом: с закрытия B-4
   * дубль проверяет сервис, и отказ называет саму дисциплину. Показываем
   * его как есть — свой перевод подменял бы сообщение сервера менее
   * точным.
   *
   * Место показа — баннер, а не поле названия: словаря `errors` в ответе
   * нет, и к какому полю относится текст, из него не следует. Заявка B-5
   * просит словарь; с ним сообщение вернётся на поле само.
   */
  it('показывает отказ по занятому названию так, как его прислал сервер', async () => {
    renderSubjects();
    ...
    expect(
      await screen.findByText('Дисциплина с названием «Базы данных» уже существует'),
    ).toBeInTheDocument();
    expect(screen.getByRole('dialog')).toBeInTheDocument();
  });
