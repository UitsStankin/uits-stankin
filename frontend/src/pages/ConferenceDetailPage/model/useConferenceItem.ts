...
import { conferenceItemQuery, findCachedConference } from '@entities/conference';
import { isApiError } from '@shared/api';
import { parseId } from '@shared/lib';

/**
 * Одно объявление: разбор адреса, запрос, подсадка данных из уже
 * загруженного списка.
 *
 * **Спиннера при переходе из списка быть не должно.** Списочная и детальная
 * ручки отдают один и тот же DTO — `content` объявления приезжает уже
 ...
 * `entities/news/lib/cachedNews.ts`.
 *
...
export function useConferenceItem() {
  ...
  const cached = id === null ? undefined : findCachedConference(queryClient, id);

  const query = useQuery({
    ...conferenceItemQuery(id ?? 0),
    ...
    enabled: id !== null,
    initialData: cached?.conference,
    initialDataUpdatedAt: cached?.updatedAt,
  });

  return {
    conference: query.data ?? null,
    ...
    /**
     * Объявления нет: либо его никогда не было, либо оно скрыто. Различить
