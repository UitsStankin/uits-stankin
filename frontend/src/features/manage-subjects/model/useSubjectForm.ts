...
function describeSaveError(
   ...
   * была открыта), занятое название, сеть, `500`. Текст ApiError пригоден
   * для показа.
   *
   * Своего перевода здесь больше нет. Занятое название приходило как `409`
   * с общим текстом «Конфликт данных.» — отказом уникального индекса базы,
   * — и форма переводила его в «Дисциплина с таким названием уже есть»,
   * опираясь на то, что уникальное поле у дисциплины ровно одно. Заявка
   * **B-4** закрыта (`c9f8597`, 2026-09-07): дубль проверяет сервис,
   * ответ — `400`, и текст в нём называет саму дисциплину. Перевод после
   * этого стал враньём наоборот — он подменял бы сообщение сервера своим,
   * менее точным.
   *
   * Цена размена: сообщение показывается баннером, а не у поля названия,
   * потому что `errors` в таком ответе нет и к какому полю оно относится,
   * из ответа не следует. Разбирать `detail` текстом — то же гадание,
   * от которого мы только что ушли. Заведена заявка **B-5**: со словарём
   * `errors` сообщение вернётся на поле само, веткой выше — её код уже
   * написан и ничего не знает про дисциплины.
   */
