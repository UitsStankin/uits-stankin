...
function describeSaveError(
  ...
  /*
   * `409` у этой ручки означает ровно одно: название занято. Проверки
   * на дубль в сервисе нет, отказ приходит от уникального индекса базы,
   * и текст у него общий — «Конфликт данных.». Показывать его дословно
   * нельзя: из него не следует ни что случилось, ни что делать.
   *
   * Перевод держится на том, что уникальное поле у дисциплины ровно одно
   * (`subject_subject.name`, docs/API.md). Появится второе — этот разбор
   * начнёт врать, и чинить его придётся здесь.
   *
   * Ошибка вешается на поле названия, а не в баннер: править надо именно
   * его, и курсор с сообщением должны быть в одном месте.
   */
  if (error.status === 409) {
    setError('name', { type: 'server', message: 'Дисциплина с таким названием уже есть' });
