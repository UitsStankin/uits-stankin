...
>
> ⚠️ Построчный разбор (§5–§7) — снимок кода **до Фазы 0**. После неё появились
> `common/` (ProblemDetail-обработчик, DTO страницы), обработка JWT-ошибок в фильтре,
> `@Transactional`, пагинация, индексы и CORS по белому списку — в разборе их нет.
...
Статусы обновлены 2026-07-27, после закрытия T-01…T-11.
...
| **D-04** | `AuthController` грузит юзера из БД дважды: `authenticate()` уже вернул `Authentication` с principal внутри. | Лишний запрос к БД на каждый логин. | ⏳ открыт → T-12 |
| **D-05** | `last_login` никогда не обновляется. | Колонка есть, писать в неё некому. | ⏳ открыт → T-12 |
...
| **D-09** | `NewsMapper.authorName` собирается без null-защиты. Уточнение: реальный симптом — строка «null Иванов» при пустом `firstName`; NPE только если `author` вовсе отсутствует. | Мусор в поле `authorName` на публичном эндпоинте. | ⏳ открыт → T-12 |
