...
| `dto/UserEventResponseDto.java`, `dto/EventUserDto.java` | Владелец и назначенные отдаются объектами с ФИО, а не голыми id, как в старом портале: иначе календарю нужен отдельный запрос за списком учёток. |
