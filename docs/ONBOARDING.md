...
**Технически:** модульный монолит на Spring Boot (9 модулей) + один Python-микросервис,
который парсит PDF расписания. Один VPS, docker compose, CI в GitHub Actions.

### Где мы сейчас

Паритет **17 модулей из 23**. Готово: аутентификация (JWT), профиль со сменой пароля
...
расписание преподавателя и сводное расписание (T-39…T-42b: разбор PDF микросервисом,
импорт админом, публичное чтение и сводная выборка с фильтром).
...
### module/schedule (описано 14 из 17 файлов)

Расписание преподавателя, T-39…T-42 и T-47. PDF кафедры разбирает не Java, а отдельный
...
| `client/ScheduleServiceClient.java` | `RestClient` к микросервису. Ошибки разведены по тому, кто чинит: `422` и `413` от сервиса становятся `400` с его текстом, обрыв связи и всё прочее — `503`. |
...
| `service/ScheduleImportService.java` | Оркестрация импорта: файл уходит в микросервис **вне** транзакции — иначе соединение с базой занято весь HTTP-вызов, до 30 секунд. |
