...
> §7 сверен с кодом 2026-08-30 и покрывает 106 файлов из 189, включая модули
...
> Разобрано 106 файлов из 189 в `src/main/java` и 4 тестовых класса из 46. Число в заголовке
...
### module/user (описано 7 из 17 файлов)
...
### module/publications (описано 12 из 15 файлов)
...
* **Тестов у всего, кроме ленты новостей.** Стенд заведён и CI гоняет его на
  каждый PR, но покрыта одна страница: вход, профиль, смена пароля, главная,
  карточка ППС и Markdown-страницы проверок не имеют, как и `shared/api`
  с обменом refresh-токена. Раскатывать по мере появления страниц — то же
  правило, что у границ ошибок (D-F9).

Осознанные долги: JWT лежит в `localStorage` (XSS уносит сессию целиком; память
плюс httpOnly refresh-cookie требуют ручки T-30), бандл в 558 КБ одним куском,
без кода-сплиттинга, и лента новостей показывает вперемешку новости и объявления
— у публичной ручки нет фильтра по `postType` (D-F7, на бэкенде это T-34).
...
нашёл в контракте, — недостающий фильтр `postType`.
