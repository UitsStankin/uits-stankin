...
Паритет **5 модулей из 23**. Готово: аутентификация (JWT), профиль и смена пароля,
роли (права проверены по всем ручкам), новости (CRUD, санитизация rich-text, превью-картинка),
редактируемые Markdown-страницы (13 разделов, T-25).
...
> Разобрано 59 файлов из 69 в `src/main/java` и 4 тестовых класса из 18. Число в заголовке
...
Статусы обновлены 2026-08-27: D-01…D-10 закрыты в T-01…T-12, D-11 найден
при сверке §7 с кодом, D-12 — при ревью контракта T-28.
...
| **D-12** | Пустая строка из формы сохраняется как есть: у необязательных строковых полей (`patronymic`, `phoneNumber`, `shortDescription` и т. п.) защиты от `""` нет, а `@Email` у Hibernate Validator считает `""` валидным адресом. | Ответы API отдают `""` там, где `API.md` обещает `null`; фронту нужны двойные проверки `null \|\| ""`. | ⏳ открыт, по плану: T-34 |
