...
Отдельно проверена работа с двумя токенами (F-15, 2026-08-28). Главное:
access-токен, истёкший по своему пятнадцатиминутному сроку, меняется
незаметно — `401`, обмен, повтор запроса, и страница открывается, как будто
ничего не было. Остальное: перезагрузка страницы восстанавливает сессию
обменом cookie; пять параллельных запросов
