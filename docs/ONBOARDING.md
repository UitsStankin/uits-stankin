...
> из 236, включая модули расписания, аспирантуры, публикаций и календаря; что осталось
...
└── .github/           workflows: тесты бэкенда, микросервиса и фронтенда, сборка
                       ...
                       dependabot.yml — еженедельные обновления и алерты (T-54f)
```
...
> Разобрано 145 файлов из 236 в `src/main/java` и 4 из 62 в `src/test/java`. Число
...
> * модуль `gradesheets` целиком — 23 файла ведомостей (T-66, T-68);
...
таблицы разные; общего — преподаватель, микросервис разбора и правило «импорт заменяет
целиком». PDF кафедры разбирает не Java, а отдельный
Python-микросервис `schedule-service/` — решение зафиксировано в ARCHITECTURE §8: готовый
парсер на `pdfplumber` уже существовал, переписывать его на Java значило бы переписывать
и его отладку на реальных файлах. Java-сторона отвечает за приём файла, пересылку,
проверку ответа и хранение.

| Файл | Что делает |
|---|---|
| `entity/Schedule.java` | Одно расписание на преподавателя, таблица `schedule_schedule` — имя из Django, иначе перенос данных `pg_dump`-ом невозможен. Коллекция пар — `LinkedHashSet` с `@OrderBy`, а не `List`: два `List` в одном join-запросе дают `MultipleBagFetchException`. |
...
| `mapper/ScheduleMapper.java` | Три перегрузки `toDto` — по `Schedule`, `ExamSchedule` и `Exam`, — плюс `toExamFilesDto` для строки со ссылками на PDF. ФИО собирает `common/FullName`, общий на три модуля. |
...
| `entity/ExamSchedule.java` | Одно расписание экзаменов на преподавателя, таблица `schedule_examschedule`. Устроено как `Schedule`: `teacher_id` уникален, коллекция с `orphanRemoval` — импорт заменяет целиком. Django-схемы здесь нет, экзамены в оригинале были только файлом. |
| `entity/Exam.java` | Экзамен: дата, время начала и конца, группа, дисциплина, аудитория. Дата и время — `LocalDate` и `LocalTime`, а не строки: год в документе экзаменов есть, в отличие от расписания пар. Дня недели нет — он выводится из даты, хранимая копия могла бы с ней разойтись. Колонка `group` экранирована так же, как у пары. |
| `entity/Consultation.java` | `@Embeddable`: три колонки консультации лежат в строке экзамена. Отдельная таблица не нужна — консультация ровно 0..1 на экзамен и отдельно не запрашивается. Hibernate отдаёт `null` вместо объекта, когда все три колонки пусты, — это ровно `"consultation": null` из ответа парсера. Целостность держит CHECK `ck_exam_consultation_all_or_none`. |
| `repository/ExamScheduleRepository.java` | То же, что у пар: `@EntityGraph` на преподавателя и экзамены, `findWithLockByTeacherId` с `PESSIMISTIC_WRITE` против одновременных импортов. |
| `service/ExamScheduleService.java` | Замена расписания экзаменов целиком, чтение одного и сводное с фильтром по группе. Разбирает ответ микросервиса: `"2025-05-14"` и `"08:30"` превращаются в типы, непонятное даёт `503`, а не 500. Фильтр по группе режет строку по запятой и сравнивает коды по отдельности — у экзамена может стоять несколько групп. Фильтруется DTO, а не коллекция сущности: правка `getExams()` в транзакции чтения означала бы `DELETE` от dirty checking. |
| `dto/ParsedExamsDto.java`, `ParsedExamDto.java`, `ParsedConsultationDto.java` | Контракт ответа `/parse-exams`. `weekDay` в контракте есть, но никуда не сохраняется — поле оставлено, чтобы DTO повторял ответ сервиса один в один. |
| `dto/ExamScheduleResponseDto.java`, `ExamResponseDto.java`, `ConsultationResponseDto.java` | Одна схема на импорт и оба публичных чтения. `consultation` может быть `null`; дня недели в схеме нет. |
...
там без единой вьюхи и без роутов, — поэтому `Student` живёт здесь как справочник,
своей ручки у него нет, и заводится он вложенно вместе с записью аспирантуры.
...
разъезжается на «Машинное обучение» и «машинное обучение».
...
| `client/SerperScholarClient.java` | Единственный поход во внешний мир в этом модуле, T-73. `POST /scholar` в Serper.dev, ключ в заголовке `X-API-KEY`, тело — record `SerperScholarRequest`. Номер страницы сдвигается на единицу: у поставщика счёт с единицы, у всех наших списков с нуля. Тело ошибки не разбирается вовсе — на таймаут приходит обычный текст, на плохой ключ JSON, — наружу летит одно исключение `ScholarUnavailableException`, которое обработчик превращает в `503`. Пустой ключ отсекается до сети: приложение обязано стартовать без него. |
| `client/SerperScholarItem.java`, `SerperScholarResponse.java`, `SerperScholarRequest.java` | Формат чужого сервиса. `year` и `citedBy` объявлены `Integer`, а не `int`: этих ключей у части записей нет, и примитив не принял бы пустоту. Незнакомые поля ответа игнорируются — поставщик добавляет их без предупреждения. |
| `client/SerperClientConfig.java` | Отдельный бин `RestClient` с базовым адресом и таймаутами. Read-timeout в 30 с не с потолка: живые замеры давали ответы до 22 с. Метод бина публичный, иначе транспортный тест из соседнего пакета его не видит. |
| `entity/ScholarSearchCache.java` | Строка кэша: нормализованный запрос, страница, ответ в `jsonb` и время получения. Уникальная пара «запрос и страница» в схеме закрывает гонку двух одинаковых поисков — второй получит ошибку ограничения, а не положит вторую строку. |
| `service/ScholarSearchService.java` | Нормализация запроса, проверки на входе, решение «база или сеть», запись свежего ответа. Не помечен `@Transactional` намеренно: внутри сетевой вызов, и держать транзакцию все 30 с ожидания нельзя — тот же приём, что в `ScheduleImportService`. Без приведения к нижнему регистру «Чеканин» и «чеканин» стали бы двумя строками и двумя потраченными кредитами. |
| `controller/ScholarSearchController.java` | `GET /api/publications/scholar` администратору и модератору. Отдельный контроллер, а не метод в `PublicationController`: тот про CRUD своей базы, этот про поиск во внешнем сервисе. |
| `dto/ScholarSearchItemDto.java`, `mapper/ScholarSearchMapper.java` | Наружу уезжает свой DTO, а не класс из `client`: формат поставщика может смениться вместе с поставщиком, контракт с фронтом от этого меняться не должен. |

### module/events (10 файлов)

Календарь событий, T-53. Событие принадлежит владельцу и назначено участникам — две
связи с `User`, и обе нужны выборке «моё или мне назначено». Модуль закрыт ролью:
наружу, в публичную зону, из него не выходит ничего.
...
| `controller/UserEventController.java` | Пять ручек под `@PreAuthorize("hasAnyRole('ADMIN', 'TEACHER')")`. Фильтр `?status=` объявлен параметром-enum'ом, поэтому значение вне словаря Spring отбивает `400` до входа в метод, а пустое считает отсутствием фильтра. |
| `mapper/UserEventMapper.java` | Владелец и назначенные превращаются в `EventUserDto`. При записи владелец, назначенные и обе колонки планировщика помечены `ignore` — их выставляет сервис, а не тело запроса. |
| `dto/UserEventRequestDto.java` | Цвет проверяется `@Pattern("^#[0-9a-fA-F]{6}$")`. `status`, `allDay` и `notificationFrequency` — объекты с `@NotNull`, а не примитивы: тело без поля должно давать `400`, а не тихо выставлять дефолт. |
| `dto/UserEventResponseDto.java`, `dto/EventUserDto.java` | Владелец и назначенные отдаются объектами с ФИО, а не голыми id, как в старом портале: иначе календарю нужен отдельный запрос за списком учёток. Логина в них нет: раздел открыт любому преподавателю, а список учёток закрыт админом, и `username` в ответе означал бы, что перебор `assignedUserIds` выгружает логины в обход этой проверки (T-54b). |
...
| API-клиент | Один экземпляр axios: `baseURL` из `.env`, `Authorization: Bearer`, `withCredentials` ради refresh-cookie, разбор `application/problem+json` в единый `ApiError` (включая словарь `errors` и `Retry-After` у `429`). На 401 — один обмен refresh-cookie на новый токен и повтор запроса; на форму входа уводит только 401 от самого `refresh` (F-15). Обмен идёт по одному на все запросы сразу: пять параллельных 401 делают один `refresh`, а не пять — второй предъявил бы уже обменянный токен, и бэкенд счёл бы это кражей. Ответ намеренно **не** разворачивается до `data`: из него нужен заголовок `Location` при создании новости. |
...
Отдельно проверена работа с двумя токенами (F-15, 2026-08-28): перезагрузка
страницы восстанавливает сессию обменом cookie; пять параллельных запросов
с негодным токеном делают **один** `refresh` и повторяются все пять; погашенная
на сервере сессия уводит на форму входа и после входа возвращает на ту же
страницу; выход отвечает `204` и стирает cookie; выход в одной вкладке
разлогинивает соседнюю без перезагрузки; шестая попытка входа за минуту
показывает отсчёт из `Retry-After` и блокирует кнопку. Гость при этом
не делает ни одного запроса к API.

Новости проверены дважды. На **заглушке контракта** — то, для чего нужен объём
данных: скелет, лента из 137 записей, многоточия пагинатора по краям и в середине,
перелистывание с прошлой страницей на экране, пустой список, обрыв связи и повтор,
обложка и картинка внутри текста, мобильная раскладка. На **живом Spring** — то,
что важно сверить с настоящим контрактом: лента, статья, переход из ленты без
единого запроса, `404`, нечисловой `id`, страница за пределами данных.
Расхождений не найдено.

Отдельно код сверен с контрактом из `origin/dev` — тот ушёл на 72 коммита
вперёд запущенного стенда. Нашлось одно расхождение, зато ломающее: `authorName`
приходит `null`, если у автора не заполнены ни имя, ни фамилия (раньше была
пустая строка). Тип объявлял `string`, и запись такого автора роняла рендер
`TypeError`, унося с экрана весь портал. Починено; заодно у страниц новостей
появился `errorElement` — до него границы ошибок в приложении не было ни одной.

Чего на живой базе не видно: в ней две записи, обе без обложки и с простым
текстом вместо HTML, поэтому пагинатор, превью и форматирование rich-text
подтверждены только на заглушке. Хранилище локального контейнера вдобавок
смотрит в `/tmp/media` без тома — картинки в нём не переживают пересоздание;
`docker-compose.prod.yml` монтирует `media_data` в `/app/media` как надо.
...
Тесты — **Vitest** (не Jest: конфиг сборки один на всё, и второго списка
алиасов FSD у тест-раннера нет), разметка — Testing Library в jsdom, запросы
перехватывает **MSW**. Браузерный воркер MSW включается флагом
`VITE_ENABLE_MOCKS=true` и показывает те же выдуманные данные, на которых
идут тесты.
...
D-12 — в T-34, D-13 — в T-35, D-03 — в T-30, D-14…D-16 — при сверке контракта
с кодом после T-31. Открытых дефектов не осталось.

Таблица ниже — дефекты **первичного разбора** июля. Находки сплошного аудита
2026-08-30 (T-54) в неё не заводились: они закрыты тем же коммитом, которым
найдены, и разобраны в бэклоге по проходам. Коротко, чтобы знать, что искали
и чем это кончилось: чистка rich-text распространена на `education`
и `qualification` карточки ППС, логин учётной записи убран из карточек календаря,
...
| **D-03** | Токен на 24 часа, refresh-токена нет, отзыва нет. | Смена пароля не инвалидирует старые токены. Противоречит ARCHITECTURE §4.5. | ✅ исправлен (T-30): access живёт 15 минут, refresh — 14 дней в httpOnly-cookie и строкой в таблице `refresh_token`; выход и повторное использование обменянного токена гасят всю цепочку сессии, смена пароля двигает `users_user.tokens_not_before` и обрывает уже выданные access-токены |
...
| **D-13** | Ключ файла проверяется через `fileStorage.exists`, который не смотрит на раздел хранилища: `NewsService`, `ConferenceService`, `AchievementService` и `HelpersEmployeeService` примут ключ любого раздела. Правка или удаление сущности затем удаляет чужой файл. Найден на ревью T-33; для аватаров закрыт там же методом `existsInCategory`. | Модератор, вставив в обложку ключ из другого раздела, сносит картинку соседней сущности; строка в БД продолжает на неё ссылаться. | ✅ исправлен (T-35): раздел сверяется через `existsInCategory` во всех сервисах, тесты на ключ из чужого раздела есть у каждой сущности |
| **D-14** | Ответ на создание записи несёт заголовок `Location`, а ответ `429` — `Retry-After`, но `CorsConfiguration.setExposedHeaders` не вызывался. Браузер отдаёт JS только безопасный набор заголовков ответа, остальные скрывает, даже когда они пришли по сети. Вдобавок `Location` собирался через `ServletUriComponentsBuilder.fromCurrentRequest()` — абсолютным, из адреса, которым запрос вошёл в приложение. Найден при сверке `API.md` с кодом. | `API.md` обещает фронту оба заголовка, а прочитать их с другого origin нельзя: `Location` вернулся бы `null`. Абсолютный адрес за nginx указывал бы на внутренний хост контейнера. | ✅ исправлен: `Location` и `Retry-After` открыты через `setExposedHeaders`, `Location` строится относительным путём `/api/news/{id}`; оба факта закрыты тестами в `CorsIntegrationTest`, проверенными на способность падать |
| **D-15** | Фоновая уборка сирот (T-31) считает занятым только файл, на который ссылается колонка обложки или аватара. Картинка, вставленная модератором в текст новости, живёт единственной ссылкой — внутри HTML в `content`, и ни одна `FileUsageProbe` туда не смотрит. Вдобавок задача обходила и раздел `publications`, у которого владельца нет вовсе. Найден при сверке `API.md` с кодом, подтверждён тестом до правки. | Через сутки после публикации картинки в тексте новости исчезают с диска навсегда, а всё загруженное в `publications` удаляется гарантированно. Потеря данных без возможности восстановления. | ✅ исправлен: пробы новостей, конференций и достижений ищут ключ и внутри `content`; уборка ограничена разделами из `ORPHAN_CLEANUP_CATEGORIES`, раздел без владельца не подметается |
| **D-16** | `newPassword` при смене пароля ограничен только снизу (`@Size(min = 8)`). BCrypt отказывается хешировать пароль длиннее 72 байт и бросает `IllegalArgumentException`, обработчика которого нет — запрос падает в `handleUnexpected`. | Пароль из менеджера паролей или парольная фраза от 37 кириллических букв (72 байта) дают `500` вместо `400`: пароль не меняется, пользователь видит «ошибка сервера», клиентской валидацией это не предотвратить. | ✅ исправлен: констрейнт `@BcryptCompatible` считает длину в байтах UTF-8 и даёт `400` со словарём `errors`; ограничение описано в `API.md` вместе с формулой подсчёта на клиенте |
