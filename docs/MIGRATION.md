...
| 2 | Профиль + смена пароля | 🟡 | правка профиля из ЛК: имя, фамилия, аватар (в старом — `update_profile`; почта там принималась формой, но сериализатор держал её read-only — правка почты только выглядела рабочей) |
| 3 | Роли (admin/moderator/teacher/user) | ✅ | — |
| 4 | Новости / Объявления | ✅ | — |
| 5 | Объявления о конференциях | ❌ | сущность + CRUD + публичный список |
| 6 | Достижения кафедры | ❌ | сущность (привязка к преподавателю), CRUD, публикация |
| 7 | Преподаватели (ППС) | 🟡 | отчество, дисциплины (M2M), enum званий/степеней, контакты, опыт, ссылки на расписание экзаменов; ручки «моя карточка» — просмотр и правка своей карточки преподавателем из ЛК (в старом — `teacher_info` / `update_teacher_info`) |
| 8 | УВП (учебно-вспом. персонал) | ❌ | сущность HelpersEmployee + список |
| 9 | Дисциплины (Subject) | ❌ | сущность + связь с преподавателем |
| 10 | Расписание преподавателя | ❌ | модель Schedule/Lesson/Date, импорт через микросервис |
...
| 14 | Аспирантура (Postgraduate) | ❌ | сущность (студент↔руководитель) |
| 15 | Научные публикации + теги | ❌ | сущности, CRUD, фильтры, ручное добавление |
...
| 21 | Загрузка файлов (аватары/превью) | 🟡 | upload с ресайзом и превью новости готовы (T-22, T-23); аватар пользователя и преподавателя сохранять некуда — поле `avatar` только читается |
