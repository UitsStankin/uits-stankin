...
| 2 | Профиль + смена пароля | ✅ | — |
| 3 | Роли (admin/moderator/teacher/user) | 🟡 | проверить покрытие во всех эндпоинтах |
| 4 | Новости / Объявления | 🟡 | rich-text контент, update/delete, превью-изображение |
...
| 7 | Преподаватели (ППС) | 🟡 | отчество, дисциплины (M2M), enum званий/степеней, контакты, опыт, ссылки на расписание экзаменов |
...
| 17 | Редактируемые Markdown-страницы (×13) | ❌ | сущность EditablePage + сидинг 13 слугов |
...
| 21 | Загрузка файлов (аватары/превью) | ❌ | upload + ресайз |
...
- Убрать `backend/.env` из git-истории, **ротировать** `POSTGRES_PASSWORD` и `JWT_SECRET_KEY`.
