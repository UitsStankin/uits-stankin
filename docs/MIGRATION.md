...
| Async / напоминания | Celery 5.4 + Redis | ⛔ напоминания сняты ([ARCHITECTURE.md](ARCHITECTURE.md) §8, пункт 14); Spring `@Scheduled` без Redis и Quartz (§4.2) остался для служебных задач — уборки файлов, токенов и счётчиков лимитов |
| Telegram | pyTelegramBotAPI (webhook) | ⛔ снято ([ARCHITECTURE.md](ARCHITECTURE.md) §8, пункт 14); проработка сохранена в §4.3 |
| Парсинг расписания | pdfplumber | **Python FastAPI микросервис** (`schedule-service/`) |
| Парсинг Excel (ведомости) | pandas / openpyxl | openpyxl в том же микросервисе (T-66a) |
...
3. **Напоминания → `@Scheduled`**, без Celery+Redis и без Quartz (минус 2 контейнера в деплое). ⛔ Снято вместе с модулем 19 (ARCHITECTURE.md §8, пункт 14).
4. **Telegram-бот → webhook** на Spring-эндпоинте с проверкой секрет-токена. ⛔ Снято тем же решением.
...
| 9 | Дисциплины (Subject) | ✅ | — (T-26: сущность, M2M-связь, список и создание; T-57a: правка и удаление) |
...
| 14 | Аспирантура (Postgraduate) | ✅ | — (T-49: `GET /api/public/postgraduates` с фильтрами по руководителю и специальности, CRUD модератору, студент заводится и удаляется вместе с записью; T-78: поиск `?q=` сразу по всем колонкам таблицы) |
...
| 16 | Интеграция Google Scholar | ✅ | — (T-73: `GET /api/publications/scholar?q=&page=` администратору и модератору, заготовки карточек с заголовком, ссылкой, сниппетом, годом и ссылкой на PDF; поставщик заменён с SerpApi на Serper.dev — регистрация в SerpApi требует телефон и отвергает российские номера, платный тариф просит иностранную карту. Три дыры оригинала закрыты: доступ сужен до модераторского, число страниц ограничено тремя, запрос уезжает JSON-телом, а не конкатенацией в URL. Ответы кэшируются в `scholar_search_cache` на месяц: пакет запросов разовый и не пополняется) |
...
`gradesheet_gradesheet`, `gradesheet_student`, `gradesheet_mark` и `gradesheet_block`
(T-66 и T-68, разобранные ведомости; таблицы разбора Excel старого портала
не переносятся, см. ниже), `scholar_search_cache` (T-73, кэш ответов Serper).
...
2. **Медиа: путь и раздел.** Django хранит путь относительно `MEDIA_ROOT`,
   ...
   и разделов шесть: `avatars`, `news`, `achievements`, `publications`, с T-79 ещё
   `staff` и `pages`; старые файлы ложатся в первые четыре.
...
### 5.1 Telegram-бот — снято (ARCHITECTURE.md §8, пункт 14)
Проработка ниже сохранена на случай возврата к модулю, в коде её нет.
...
Бэкендная часть всех фаз закрыта; открытым остаётся модуль 23 — React-админка на фронтенде.
...
- **Фаза 3 — Корпоративная часть:** календарь событий → планировщик → Telegram-бот (вместе). Планировщик и бот сняты (ARCHITECTURE.md §8, пункт 14).
- **Фаза 4 — Наука:** публикации + теги + Scholar.
- **Фаза 5 — Ведомости:** подсистема Excel parse/export (можно отложить/вынести). Разбор сделан в T-66 и T-68, генерация бланков снята (ARCHITECTURE.md §8, пункт 13).

---

## 7. Баги действующего кода — НЕ переносить, исправить

1. `User.save()` пересоздаёт `telegram_code` при каждом сохранении → ломает привязку. Генерировать один раз. ⛔ Неприменимо: Telegram-бот снят (ARCHITECTURE.md §8, пункт 14).
2. `UserEvent.next_notification_at = auto_now=True` затирает плановую дату. Убрать `auto_now`. ✅ закрыто в T-53a.
3. Парсер расписания берёт только последнюю страницу PDF (цикл перезаписывает `table`). ✅ закрыто в T-39.
4. Celery-интервалы фиксированы в секундах, «месяц» = 28 дней — переделать на нормальное расписание. ⛔ Неприменимо: напоминания сняты (там же).
...
6. Публикации хранят PDF как base64 в `TEXT` — выносить в хранилище. ✅ закрыто в T-50a и T-50c.
7. Десятки `print()`-отладок и сырые потоки на каждый апдейт/сообщение — заменить на логгер/`@Async`.
8. `update_profile` писал всё, что не перечислено в `read_only_fields`, включая `is_staff` (любой пользователь мог выдать себе доступ в Django-админку) и `telegram_code`; заодно позволял менять `username` — то есть логин. В новом портале из ЛК правятся только имя, фамилия и аватар; логин и почта из профиля не меняются. ✅ закрыто в T-33.
...
- ~~Убрать `backend/.env` из git-истории~~ — проверено 2026-07-27: `backend/.env` в историю
  ...
  в секретах environment `production` (T-43), файл `.env` на сервере пишет деплой (T-84);
  сами прод-секреты заводятся в T-17d.
- ~~Сузить CORS в `SecurityConfig` до доменов фронтенда~~ — сделано в T-08: список origin
  задаётся `CORS_ALLOWED_ORIGINS`, в прод-контуре переменная обязательна.
- ~~Проверить `@PreAuthorize` на всех непубличных эндпоинтах~~ — сделано в T-24: матрица
  «роль × ручка» в `EndpointAccessMatrixTest`.
...
```
uits-stankin/
├── backend/            # Spring Boot (модули: auth, user, news, staff, achievements, students, schedule, events, publications, pages, gradesheets); docker-compose*.yml, ops/
├── frontend/           # React (public-зона + админка); Dockerfile и nginx.conf прод-образа
├── schedule-service/   # Python FastAPI: разбор PDF расписаний и Excel-ведомостей
├── docs/               # MIGRATION.md (этот файл) и остальная документация
